...
xmlSAXHandlerPtr _debugSAXHandler = &debugSAXHandlerStruct;
...
parseAndPrintFile(char *filename) {
	...
	if (f != NULL) {
	    ...
	    if (ret > 0) {
		    ...
		    ctxt = xmlCreatePushParserCtxt(_debugSAXHandler, NULL,
				chars, ret, filename);
		while ((ret = fread(chars, 1, 3, f)) > 0) {
		    xmlParseChunk(ctxt, chars, ret, 0);
		}
	...
	if (!speed) {
		...
		for (i = 0;i < 99;i++) {
			...
			res = xmlSAXUserParseFile(_debugSAXHandler, NULL,
			                          filename);
		}
	    }
	    if (sax2)
		...
		res = xmlSAXUserParseFile(_debugSAXHandler, NULL, filename);
	    if (res != 0) {
		fprintf(stdout, "xmlSAXUserParseFile returned error %d\n", res);
	    }
	    if (quiet)
...
int libxml2_test_sax_main(int argc, char **argv) {
    int i;
    int files = 0;
