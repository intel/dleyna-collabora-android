...
int libxml2_test_xpath_main(int argc, char **argv) {
    int i;
    int strings = 0;
    int usefile = 0;
    char *filename = NULL;
...
#else
#include <stdio.h>
int libxml2_test_xpath_main(int argc ATTRIBUTE_UNUSED, char **argv ATTRIBUTE_UNUSED) {
    fprintf(stderr, "%s : XPath/Debug support not compiled in\n", argv[0]);
    return(0);
}
