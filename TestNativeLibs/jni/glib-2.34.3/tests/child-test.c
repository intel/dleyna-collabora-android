...
int
glib_child_test_main (int argc, char *argv[])
{
#ifndef TEST_THREAD
  GPid pid;
#endif
...
#ifdef TEST_THREAD
  g_thread_create (test_thread, GINT_TO_POINTER (TTL), FALSE, NULL);
  g_thread_create (test_thread, GINT_TO_POINTER (2*TTL), FALSE, NULL);
#else
  pid = get_a_child (TTL);
  g_child_watch_add (pid, (GChildWatchFunc) child_watch_callback,
		     GINT_TO_POINTER (TTL));
  pid = get_a_child (2*TTL);
  g_child_watch_add (pid, (GChildWatchFunc) child_watch_callback,
		     GINT_TO_POINTER (2*TTL));
#endif
