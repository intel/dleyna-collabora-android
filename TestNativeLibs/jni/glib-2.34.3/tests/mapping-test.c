...
g_print("test_child_private: ENTER\n"); sleep(1);
  ...
  write_or_die (filename, "ABC", -1);
  map = map_or_die (filename, TRUE);
g_print("test_child_private: mapped\n"); sleep(1);
...
g_print("test_child_private: spawned child\n"); sleep(1);
  ...
  g_assert (strcmp (buffer, "ABC") == 0);
  g_free (buffer);
g_print("test_child_private: EXIT\n");
}
...
parent_main (int   argc,
	     char *argv[])
{
g_print("parent-main: test_mapping\n"); sleep(1);
  /* test mapping with various flag combinations */
  test_mapping ();

g_print("parent-main: test_private\n"); sleep(1);
  /* test private modification */
  test_private ();

g_print("parent-main: test_child_private\n"); sleep(1);
...
int
glib_mapping_test_main (int argc, 
      char *argv[])
{
